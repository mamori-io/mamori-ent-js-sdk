...
import { io_https, io_utils, io_permission, io_role, io_http_resource } from '../../api';
...
describe("secret permission tests", () => {
    ...
    let resource = "http_res-" + testbatch;
    let permType = "HTTP ACCESS";
    let grantee = "test_apiuser_httpr." + testbatch;
    ...
    beforeAll(async () => {
        ...
        let newS = new io_http_resource.HTTPResource(resource)
            .withURL("https://mamoritest.io/login")
            .withDescription("Test web resource");
        await io_utils.ignoreError(newS.create(api));

        let newCapS = new io_http_resource.HTTPResource('CAPS_' + resource)
            .withURL("https://mamoritest.io/login2")
            .withDescription("Test web resource 2");
        await io_utils.ignoreError(newCapS.create(api));
    });

    afterAll(async () => {
        await io_utils.ignoreError(new io_http_resource.HTTPResource("CAPS" + resource).delete(api));
        await io_utils.ignoreError(new io_http_resource.HTTPResource(resource).delete(api));
        await io_utils.ignoreError(api.delete_user(grantee));
        await api.logout();
    ...
    test('grant 01', async () => {
        let obj = new io_permission.HTTPResourcePermission()
            .name(resource)
            .grantee(grantee);
        //make sure no exist
        ...
        let res = await new io_permission.HTTPResourcePermission().grantee(grantee).list(api, filter);
        expect(res.totalCount).toBe(0);
        //grant
        let resp = await io_utils.noThrow(obj.grant(api));
        expect(resp.errors).toBe(false);
        //check list
        res = await new io_permission.HTTPResourcePermission().grantee(grantee).list(api, filter);
    ...
    test('grant 02', async () => {
        ...
        let r1 = await new io_permission.HTTPResourcePermission()
            .name(resource)
            .grantee(grantee)
        ...
        let r2 = await io_utils.noThrow(new io_permission.HTTPResourcePermission().revokeByID(api, id));
    ...
    test('grant 03 - grant between', async () => {
        ...
        let obj = await new io_permission.HTTPResourcePermission()
            .name(resource)
            .grantee(grantee)
        ...
        let res2 = await new io_permission.HTTPResourcePermission().grantee(grantee).list(api, filter);
    ...
    test('grant 04 - mixed case', async () => {
        ...
        let objMixedCase = new io_permission.HTTPResourcePermission()
            .name(name)
            .grantee(grantee);
        let objLower = new io_permission.HTTPResourcePermission()
        ...
        let filter = [["permissiontype", io_utils.FILTER_OPERATION.EQUALS_STRING, permType],
        ["grantee", io_utils.FILTER_OPERATION.EQUALS_STRING, grantee]];
        let r5 = await io_utils.noThrow(new io_permission.HTTPResourcePermission().grantee(grantee).list(api, filter));
    ...
    test('test 05 role grant', async () => {
        ...
        res = await new io_permission.HTTPResourcePermission().grantee(roleName).list(api, f);
        expect(res.totalCount).toBe(1);
