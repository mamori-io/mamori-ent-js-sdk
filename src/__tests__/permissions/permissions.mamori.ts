...
import { MamoriPermission, MAMORI_PERMISSION, TIME_UNIT } from '../../../dist/permission';
...
describe("mamori permission tests", () => {
    ...
    test('revoke 01', async done => {
        try {
            let resp = await new MamoriPermission()
    ...
    test('grant 02', async done => {
        try {
            ...
            let r2 = await new MamoriPermission().revokeByID(api, id);
