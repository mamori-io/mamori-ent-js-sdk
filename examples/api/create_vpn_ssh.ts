...
let eg = async function (dm: DMService, args: ParsedArgs) {
  ...
  console.info(`VPN: ${vpnName} -i ${keyFile} -L ${local_port}:${destination}:${destination_port} ${user}@${server}`);
  ...
  await dm.create_vpn_connection({
    ...
    secrets: [{name: "privateKey", value: privateKey}]
  });
}
...
rapt.usage =
"Usage:\n" + 
"   yarn ts-node <example script> [--help] [--url <url>] [<user> <password>] vpnName  -i identity_file -L local_port:desination:desination_port user@ssh_server\n" + 
"where:\n" + 
